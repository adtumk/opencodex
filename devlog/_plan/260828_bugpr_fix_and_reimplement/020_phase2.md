...
```ts
/**
 * The validator-bypass sentinel is a Gemini wire contract. The replay cache
 * deliberately spans every non-Claude model, so cache scope cannot gate it: that is
 * how a Gemini-only token reached gpt-oss-120b-medium. Match the transport-scoped
 * Vertex namespace too, since the same wire dialect arrives under a prefixed id.
 */
function antigravitySupportsThoughtSignatureSentinel(model: string): boolean {
